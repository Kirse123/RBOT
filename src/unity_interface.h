...
extern "C" int RBOT_API EstimatePose(float* outPoseData, bool undistortFrame, bool checkForLoss);

///<summary>Initiate RBOT vars and starts QApplication. Should be called when object3d are added</summary>
extern "C" int RBOT_API CreatePoseEstimator(int camera_width, int camera_height, float inZNear, float inZFar, float* inK, float* inDistCoeffs, float* inDistances);
